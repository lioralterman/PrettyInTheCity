...
class Login {
    ...
    private function loginWithPostData() {
        ...
        if (!empty($_POST['user_email']) && !empty($_POST['user_password'])) {
            ...
            if (!$this->db_connection->connect_errno) {
                ...
                $checklogin = $this->db_connection->query("SELECT user_id, user_name, user_email, user_password_hash, user_active, user_image FROM users WHERE user_email = '".$this->user_email."';");
                ...
                if ($checklogin->num_rows == 1) {
                    ...
                    if (password_verify($_POST['user_password'], $result_row->user_password_hash)) {
                        ...
                        if ($result_row->user_active == 1) {
							...
							$_SESSION['user_birth'] = $result_row->user_birth;
    ...
    public function setPasswordResetDatabaseToken() {
        ...
        if (empty($_POST['user_email'])) {
            ...
            if (!$this->db_connection->connect_errno) {
                ...
                $this->user_email = $this->db_connection->real_escape_string(htmlentities($_POST['user_name'], ENT_QUOTES));                
                $query_get_user_data = $this->db_connection->query("SELECT user_id, user_email FROM users WHERE user_email = '".$this->user_email."';");
    ...
    public function sendPasswordResetMail() {
        ...
        $link    = EMAIL_PASSWORDRESET_URL.'?user_email='.urlencode($this->user_email).'&verification_code='.urlencode($this->user_password_reset_hash);
    ...
    public function checkIfEmailVerificationCodeIsValid() {
        ...
        if (!empty($_GET["user_email"]) && !empty($_GET["verification_code"])) {
            ...
            if (!$this->db_connection->connect_errno) {
                ...
                // TODO: this is not totally clean, as this is just the form provided username
                $this->user_email                = $this->db_connection->real_escape_string(htmlentities($_GET['user_email'], ENT_QUOTES));         
