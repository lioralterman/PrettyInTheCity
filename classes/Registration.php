...
class Registration {
	...
	private     $user_birth                 = "";                       // user's birth date
	private     $user_sex                   = "";                       // user's sex
	private     $user_interest              = "";                       // user's interest
    ...
    private function registerNewUser() {
        ...
        if (empty($_POST['user_first_name'])) {
		...
		} elseif (empty($_POST['user_last_name']) || empty($_POST['user_password_repeat'])) {
        ...
        } elseif (strlen($_POST['user_first_name']) > 64 || strlen($_POST['user_first_name']) < 2) {
            
            $this->errors[] = "First name cannot be shorter than 2 or longer than 64 characters";
                        
        } elseif (!preg_match('/^[a-z\d]{2,64}$/i', $_POST['user_first_name'])) {
            
            $this->errors[] = "First name does not fit the name sheme: only a-Z and numbers are allowed, 2 to 64 characters";
			
		} elseif (strlen($_POST['user_last_name']) > 64 || strlen($_POST['user_last_name']) < 2) {
            
            $this->errors[] = "Last name cannot be shorter than 2 or longer than 64 characters";
                        
        } elseif (!preg_match('/^[a-z\d]{2,64}$/i', $_POST['user_last_name'])) {
            
            $this->errors[] = "Last name does not fit the name sheme: only a-Z and numbers are allowed, 2 to 64 characters";
            ...
            $this->errors[] = "Your email address is not in a valid email format";
                  ...
                  && strlen($_POST['user_first_name']) <= 64
                  && strlen($_POST['user_first_name']) >= 2
                  && preg_match('/^[a-z\d]{2,64}$/i', $_POST['user_first_name'])
            ...
            if (!$this->db_connection->connect_errno) {
                ...
                $this->user_name            = $this->db_connection->real_escape_string(htmlentities($_POST['user_first_name']." ".$_POST['user_last_name'], ENT_QUOTES));
                $this->user_email           = $this->db_connection->real_escape_string(htmlentities($_POST['user_email'], ENT_QUOTES));
				$this->user_birth            = $this->db_connection->real_escape_string(htmlentities($_POST['user_birth_year']."-".$_POST['user_birth_month']."-".$_POST['user_birth_day'], ENT_QUOTES));
				if($_POST['user_sex']=="male")
				$this->user_sex           = "m";
				else if($_POST['user_sex']=="female")
				$this->user_sex           = "f";
				if($_POST['user_interest']=="men")
				$this->user_interest           = "m";
				else if($_POST['user_interest']=="women")
				$this->user_interest           = "f";
                ...
                $query_check_email = $this->db_connection->query("SELECT * FROM users WHERE user_email = '".$this->user_email."';");
                ...
                if ($query_check_email->num_rows == 1) {
                    ...
                    $query_new_user_insert = $this->db_connection->query("INSERT INTO users (user_name, user_password_hash, user_email, user_activation_hash, user_sex, user_birth, user_interest, user_rating) VALUES('".$this->user_name."', '".$this->user_password_hash."', '".$this->user_email."', '".$this->user_activation_hash."', '".$this->user_sex."', '".$this->user_birth."', '".$this->user_interest."',1000);");
